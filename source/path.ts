...
export function ensureTriggerChild(trigger: string | RegExpLike): void {
	...
	if (source.includes('/') || source.startsWith('..')) {
		throw new Error(
			'actions are not relative. They have to be below the menu (dont contain / or ..)',
		);
	}
}
...
export function ensureTriggerLastChild(trigger: string | RegExpLike): void {
	...
	if (!source.endsWith('$')) {
		throw new Error(
			'actions are meant for buttons, the leaves of the menu tree. Regex ends with $: /button$/',
		);
	}
}
...
export function combineTrigger(
	...
	if (!parent.source.startsWith('^')) {
		throw new Error(
			'the path has to begin from start in order to prevent mistakes: /^something…\\//',
		);
	}

	if (!parent.source.endsWith('/')) {
		throw new Error(
			'the path of the menu has to be supplied. Menu paths end with /',
		);
	}
...
export function createRootMenuTrigger(
	...
	if (typeof rootTrigger === 'string') {
		...
		if (count !== 1) {
			throw new Error(
				'The root menu trigger can only trigger exactly one slash as each slash represents a submenu /',
			);
		}
	}
...
export function ensurePathMenu(path: string): void {
	...
	if (!path.endsWith('/')) {
		throw new Error(
			'the path of a menu has to be supplied. Menu paths end with /',
		);
	}
}
...
export function combinePath(parent: string, relativePath: string): string {
	...
	if (relativePath.startsWith('./')) {
		throw new Error(
			'the path can start without prefixing it with ./ When navigating to the current menu use an empty string or . (dot)',
		);
	}
...
export function getMenuOfPath(path: string): string {
	if (!path.includes('/')) {
		throw new Error(
			'This does not seem like a path. Paths contain slashes to separate the submenus.',
		);
	}
